...
class MxResNet:
    ...
    def residual_module(data, K, stride, red=False, bnEps=2e-5, bnMom=0.9):
        ...
        conv2 = mx.sym.Convolution(data=act2, pad=(1, 1), kernel=(3, 3), stride=stride, num_filter=(K * 0.25),
                                   no_bias=True)
    ...
    def build(classes, stages, filters, bnEps=2e-5, bnMom=0.9):
        ...
        conv1_1 = mx.sym.Convolution(data=bn1_1, pad=(3, 3), kernel=(7, 7), stride=(2, 2), num_filter=filters[0],
                                     no_bias=True)
        bn1_2 = mx.sym.BatchNorm(data=conv1_1, fix_gamma=True, eps=bnEps, momentum=bnMom)
